...
export function donutSVG(segments, { width = 440, height = 240, centerLabel = '', centerSub = '' } = {}) {
  const segs = (segments || []).filter(s => s && s.value > 0)
  const total = segs.reduce((s, x) => s + x.value, 0)
  const cx = 118, cy = height / 2, r = 80, sw = 30
  const C = 2 * Math.PI * r
  ...
  const center = centerLabel
    ? `<text x="${cx}" y="${cy - 1}" text-anchor="middle" font-family="${FONT}" font-size="32" font-weight="700" fill="${PALETTE.text}">${esc(centerLabel)}</text>` +
      `<text x="${cx}" y="${cy + 20}" text-anchor="middle" font-family="${FONT}" font-size="12" fill="${PALETTE.muted}">${esc(centerSub)}</text>`
    : ''

  // Legend (right side)
  const allSegs = segments || []
  let ly = cy - (allSegs.length * 24) / 2 + 8
  let lg = ''
  for (const seg of allSegs) {
    const pct = total > 0 ? Math.round((seg.value / total) * 100) : 0
    lg += `<rect x="244" y="${(ly - 11).toFixed(1)}" width="12" height="12" rx="3" fill="${seg.color}"/>`
    lg += `<text x="263" y="${ly.toFixed(1)}" font-family="${FONT}" font-size="13" fill="${PALETTE.text}">${esc(seg.label)}</text>`
    lg += `<text x="${width - 10}" y="${ly.toFixed(1)}" text-anchor="end" font-family="${FONT}" font-size="13" font-weight="700" fill="${PALETTE.muted}">${seg.value} · ${pct}%</text>`
    ly += 24
  }
...
export function columnSVG(items, { width = 660, height = 300 } = {}) {
  ...
  const padL = 40, padR = 12, padT = 30, padB = 66
  ...
  data.forEach((d, i) => {
    ...
    bars += `<text x="${gx.toFixed(1)}" y="${baseline + 13}" transform="rotate(45 ${gx.toFixed(1)} ${baseline + 13})" font-family="${FONT}" font-size="9" fill="${PALETTE.muted}">${esc(truncate(d.label, 14))}</text>`
  })
...
// ── Horizontal bars: distribution (modules / assignees / phase progress) ────
// items: [{ label, value, sub }]
//   format 'hours' → value in seconds, shown as "12.3h"
//   format 'pct'   → value 0..1, shown as "67%" (pass max:1)
export function hbarSVG(items, { width = 640, color = PALETTE.accent, format = 'hours', max = null } = {}) {
  const data = (items || []).slice(0, 12)
  if (!data.length) return emptySVG(width, 80)

  const rowH = 28, padT = 10, padB = 10
  const height = padT + padB + data.length * rowH
  const labelW = 180, valW = 92
  const barMaxW = width - labelW - valW - 14
  const maxV = max != null ? max : Math.max(...data.map(d => d.value || 0), 1)

  let rows = ''
  data.forEach((d, i) => {
    const cy = padT + i * rowH + rowH / 2
    const bw = Math.max(2, ((d.value || 0) / maxV) * barMaxW)
    const valTxt = format === 'pct'
      ? Math.round((d.value || 0) * 100) + '%'
      : ((d.value || 0) / 3600).toFixed(1) + 'h'
    ...
    rows += `<text x="${width}" y="${(cy + 4).toFixed(1)}" text-anchor="end" font-family="${FONT}" font-size="11" font-weight="700" fill="${PALETTE.muted}">${esc(valTxt)}${d.sub ? ' · ' + esc(d.sub) : ''}</text>`
  })
