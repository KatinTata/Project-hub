...
export default function PhaseForecast({ tasks, phases, canEditConfig }) {
  ...
  const [basis, setBasis] = useState('remaining') // 'remaining' | 'plan'
  ...
  const forecast = useMemo(
    () => (settings ? buildPhaseForecast(matrix, settings, { basis, peoplePerStack: people }) : null),
    [matrix, settings, basis, people]
  )
      ...
      <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div>
          ...
          <div style={{ ...labelMuted, marginTop: 2 }}>
            Napor (čovek-dani) → trajanje (uz {people} {people === 1 ? 'osobu' : 'osobe'}/stek, paralelno) → ulančani datumi
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {/* people per stack */}
          <label style={{ display: 'flex', alignItems: 'center', gap: 5, ...labelMuted }}>
            ljudi/stek
            <input type="number" min={1} max={20} value={people}
              onChange={e => setPeople(Math.max(1, parseInt(e.target.value, 10) || 1))}
              style={{ width: 46, padding: '4px 6px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text)', fontFamily: "'DM Mono'", fontSize: 12 }} />
          </label>
        </div>
      </div>
        ...
        <>
          ...
          <div style={{ padding: '12px 16px', background: 'var(--surfaceAlt)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
            <span style={labelMuted}>
              ...
              {basis === 'remaining' ? ' (na preostali napor, od danas)' : ' (pun plan, od danas)'}
            </span>
