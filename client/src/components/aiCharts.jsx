...
export function PieChart({ data, size = 170, thickness = 26, centerLabel, centerValue, valueFmt = v => v, max = 8 }) {
  ...
  if (!total) return <div style={{ padding: 18, color: 'var(--textSubtle)', fontFamily: "'DM Sans', sans-serif", fontSize: 12 }}>Nema podataka</div>
    ...
    <div style={{ display: 'flex', gap: 18, alignItems: 'center', flexWrap: 'wrap' }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ flexShrink: 0 }}>
        ...
        {(centerValue != null || centerLabel) && (
          <>
            <text x={c} y={c - 2} textAnchor="middle" fontFamily="Syne" fontWeight="800" fontSize="17" fill="var(--text)">{centerValue}</text>
            <text x={c} y={c + 15} textAnchor="middle" fontFamily="'DM Sans', sans-serif" fontSize="10" fill="var(--textMuted)">{centerLabel}</text>
          </>
        )}
      </svg>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, minWidth: 150, flex: 1 }}>
        {segs.map((d, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, fontFamily: "'DM Sans', sans-serif", fontSize: 12 }}>
            ...
            <span style={{ fontFamily: "'DM Mono'", fontSize: 11, color: 'var(--textMuted)' }}>{Math.round(d.value / total * 100)}%</span>
            <span style={{ fontFamily: "'DM Mono'", fontSize: 11, color: 'var(--text)', minWidth: 54, textAlign: 'right' }}>{valueFmt(d.value)}</span>
          </div>
        ))}
      </div>
    </div>
...
export function HBars({ data, valueFmt = v => v, max = 10, color }) {
  ...
  if (!rows.length) return <div style={{ padding: 18, color: 'var(--textSubtle)', fontFamily: "'DM Sans', sans-serif", fontSize: 12 }}>Nema podataka</div>
  const peak = Math.max(...rows.map(d => d.value))
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {rows.map((d, i) => (
        <div key={i}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, fontFamily: "'DM Sans', sans-serif", fontSize: 12, marginBottom: 3 }}>
            <span style={{ color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={d.label}>{d.label}</span>
            <span style={{ fontFamily: "'DM Mono'", fontSize: 11, color: 'var(--textMuted)', flexShrink: 0 }}>{valueFmt(d.value)}</span>
          </div>
...
export function TrendChart({ days, currency = 'USD', height = 220 }) {
  const rows = days || []
  if (!rows.length) return <div style={{ padding: 24, textAlign: 'center', color: 'var(--textMuted)', fontFamily: "'DM Sans', sans-serif", fontSize: 13 }}>Nema podataka za period</div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 16, marginBottom: 6, fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: 'var(--textMuted)' }}>
      ...
      <svg viewBox={`0 0 ${W} ${height}`} style={{ width: '100%', height: 'auto' }}>
        {[0, 0.25, 0.5, 0.75, 1].map(t => (
          <g key={t}>
            ...
            <text x={padL - 7} y={padT + plotH * t + 4} textAnchor="end" fontFamily="DM Mono" fontSize="9.5" fill="var(--textMuted)">{fmtMoney(maxCost * (1 - t), currency)}</text>
            <text x={W - padR + 7} y={padT + plotH * t + 4} textAnchor="start" fontFamily="DM Mono" fontSize="9.5" fill="var(--textMuted)">{fmtTok(maxReq * (1 - t))}</text>
          </g>
        ))}
        {rows.map((d, i) => {
        ...
        {rows.map((d, i) => (i % step === 0 || i === rows.length - 1) && (
          <text key={d.date} x={x(i)} y={height - 8} textAnchor="middle" fontFamily="DM Mono" fontSize="9.5" fill="var(--textMuted)">{String(d.date).slice(5)}</text>
        ))}
      </svg>
    </div>
  )
...
export function BudgetGauge({ name, spent, limit, pct, level }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 4, fontFamily: "'DM Sans', sans-serif", fontSize: 12 }}>
        <span style={{ color: 'var(--text)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</span>
        <span style={{ fontFamily: "'DM Mono'", fontSize: 11, color }}>{fmtMoney(spent, 'EUR')} / {fmtMoney(limit, 'EUR')} · {p}%</span>
      </div>
